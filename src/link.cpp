  ...
  Inertial Inertial::fromXml(TiXmlElement *xml) {
		...
		if (mass_xml != nullptr) {
			if (mass_xml->Attribute("value") != nullptr) {
				try {
					i.mass = StringTool::castStringToDouble(mass_xml->Attribute("value"));
		...
		if (inertia_xml != nullptr) {
			if (inertia_xml->Attribute("ixx") && inertia_xml->Attribute("ixy") && inertia_xml->Attribute("ixz") &&
				...
				try {
					i.ixx	= StringTool::castStringToDouble(inertia_xml->Attribute("ixx"));
					i.ixy	= StringTool::castStringToDouble(inertia_xml->Attribute("ixy"));
					i.ixz	= StringTool::castStringToDouble(inertia_xml->Attribute("ixz"));
					i.iyy	= StringTool::castStringToDouble(inertia_xml->Attribute("iyy"));
					i.iyz	= StringTool::castStringToDouble(inertia_xml->Attribute("iyz"));
					i.izz	= StringTool::castStringToDouble(inertia_xml->Attribute("izz"));
				} catch (StringToolException &e) {
					std::ostringstream error_msg;
					error_msg << "Error while parsing link '" << getParentLinkName(xml)
